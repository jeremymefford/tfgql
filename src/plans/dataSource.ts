...
export class PlansAPI {
  ...
  private async createPlanExport(planId: string, dataType = 'sentinel-mock-bundle-v0') {
  ...
  private async findExistingPlanExportId(planId: string, dataType = 'sentinel-mock-bundle-v0'): Promise<string | null> {
    ...
    for (const ref of refs) {
      ...
      try {
        ...
        const isUsable = exp.attributes?.status && exp.attributes.status !== 'expired';
  ...
  private async ensurePlanExport(planId: string, dataType = 'sentinel-mock-bundle-v0'): Promise<string> {
    try {
  ...
  private async waitForPlanExportFinished(planExportId: string, pollIntervalMs = 1000, timeoutMs = 5 * 60 * 1000) {
    ...
    while (true) {
      ...
      if (status === 'errored' || status === 'canceled' || status === 'expired') {
        throw new Error(`Plan export ${planExportId} is ${status}`);
      }
      if (Date.now() - start > timeoutMs) {
        throw new Error(`Timed out waiting for plan export ${planExportId} to finish`);
      }
      await new Promise(res => setTimeout(res, pollIntervalMs));
    }
  }

  async getPlanExportDownloadUrl(planId: string, options?: { pollIntervalMs?: number; timeoutMs?: number }): Promise<string> {
    return this.requestCache.getOrSet<string>(`PlanExportDownloadUrl`, planId, async () => {

      const planExportId = await this.ensurePlanExport(planId);
      const exp = await this.waitForPlanExportFinished(planExportId, options?.pollIntervalMs, options?.timeoutMs);
      ...
      const location = (resp.headers as any)?.location || (resp.headers as any)?.Location;
