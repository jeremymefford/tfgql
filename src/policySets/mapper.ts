    ...
    return {
      ...
      vcsRepo: attrs['vcs-repo']
        ? {
            ...
            githubAppInstallationId: attrs['vcs-repo']['github-app-installation-id']
          }
        : undefined,
      ...
      workspaceIds: resource.relationships?.workspaces?.data.map(r => r.id) ?? [],
      workspaceExclusionIds:
        resource.relationships?.['workspace-exclusions']?.data.map(r => r.id) ?? [],
