...
import { PolicyEvaluationFilter } from "../policyEvaluations/types";
...
export const resolvers = {
      ...
      for (const orgId of orgs) {
        for await (const workspacePage of ctx.dataSources.workspacesAPI.listWorkspaces(
          ...
          await parallelizeBounded(workspacePage, async (workspace) => {
            const workspaceRuns = await gatherAsyncGeneratorPromises(
              ctx.dataSources.runsAPI.listRuns(workspace.id, filter),
            );
            for (const run of workspaceRuns) {
              let validRun = true;
              if (planFilter) {
                const plan = await ctx.dataSources.plansAPI.getPlanForRun(
                  run.id,
                );
                validRun &&= plan
                  ? evaluateWhereClause(planFilter, plan)
                  : false;
              }
              if (applyFilter) {
                const apply = await ctx.dataSources.appliesAPI.getRunApply(
                  run.id,
                );
                validRun &&= apply
                  ? evaluateWhereClause(applyFilter, apply)
                  : false;
              }
              if (validRun) {
                result.push(run);
          ...
          for (const workspace of workspacePage) {
            for await (const runPage of ctx.dataSources.runsAPI.listRuns(
              ...
              await parallelizeBounded(runPage, async (run) => {
                const policyChecks =
                  await ctx.dataSources.policyChecksAPI.listPolicyChecks(
                    run.id,
                  );
                const policyEvals = await getPolicyEvaluationsForRun(
                  run,
                  ctx,
                  undefined,
                );
                if (
                  policyChecks.some(
                    (check) => check.statusTimestamps.overriddenAt,
                  ) ||
                  policyEvals.some(
                    (policyEval) => policyEval.status === "overridden",
                  )
                ) {
                  runs.push(run);
                }
  ...
  Run: {
    ...
    policyEvaluations: (
      run: Run,
      args: { filter?: PolicyEvaluationFilter },
      ctx: Context,
