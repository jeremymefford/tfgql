...
async function prepareRuntime() {
  ...
  const entrySource = [
    ...
    'const ENTRY_ASSET = "bundle/index.js";',
    "",
    "function runAsset(assetPath) {",
    "  const raw = getAsset(assetPath);",
    "  if (!raw) {",
    '    throw new Error(`Missing SEA asset: ${assetPath}`);',
    "  }",
    "  const asset = Buffer.isBuffer(raw)",
    "    ? raw",
    "    : Buffer.from(raw instanceof ArrayBuffer ? new Uint8Array(raw) : raw);",
    "  const filename = path.resolve(process.cwd(), assetPath);",
    "  const mod = new Module(filename, null);",
    "  require.main = mod;",
    "  process.mainModule = mod;",
    "  mod.filename = filename;",
    "  mod.paths = Module._nodeModulePaths(path.dirname(filename));",
    "  mod._compile(asset.toString(\"utf8\"), filename);",
    "}",
    "",
    "startupSnapshot.setDeserializeMainFunction(() => runAsset(ENTRY_ASSET));",
    "",
    'if (process.env.NODE_SEA_BUILD !== "1") {',
    "  runAsset(ENTRY_ASSET);",
    "}",
    "",
  ].join("\n");
