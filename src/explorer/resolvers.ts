...
>(
  ...
  queryFn: (
    org: string,
    opts: ExplorerQueryOptions<Field>,
  ) => Promise<{ data: R[] }>,
): Promise<R[]> {
...
async function fetchWorkspacesByNames(
  ...
  const resolved = await parallelizeBounded(
    uniqueNames,
    async (workspaceName) =>
      await ctx.dataSources.workspacesAPI.getWorkspaceByName(
        orgName,
        workspaceName,
      ),
  );
  ...
  return resolved
    ...
    .filter(
      (workspace) =>
        !filter ||
        evaluateWhereClause<Workspace, WorkspaceFilter>(filter, workspace),
    );
}
...
export const resolvers = {
  Query: {
    explorerWorkspaces: async (
      ...
      const options = buildOptions({ sort: args.sort, filters: args.filters });
      return executeExplorerQuery(
        ctx,
        args.includeOrgs,
        args.excludeOrgs,
        options,
        (org, opts) => ctx.dataSources.explorerAPI.queryWorkspaces(org, opts),
      );
    },
    ...
    explorerProviders: async (
      ...
      return executeExplorerQuery(
        ...
        args.excludeOrgs,
        options,
        (org, opts) => ctx.dataSources.explorerAPI.queryProviders(org, opts),
      );
    },
    ...
    explorerModules: async (
      ...
      return executeExplorerQuery(
        ...
        args.excludeOrgs,
        options,
        (org, opts) => ctx.dataSources.explorerAPI.queryModules(org, opts),
      );
    },
  },
  ExplorerWorkspaceRow: {
    workspace: async (
      row: ExplorerWorkspaceRow,
      _: unknown,
      ctx: Context,
  ...
  ExplorerTerraformVersionRow: {
    ...
    organization: async (
      ...
      return ctx.dataSources.organizationsAPI.getOrganization(orgName);
    },
  },
  ExplorerProviderRow: {
    workspaceEntities: async (
