...
export class ProjectsAPI {
    ...
    async getProject(id: string): Promise<Project | null> {
        return this.requestCache.getOrSet<Project | null>(
            ...
            id,
            async () =>
                this.httpClient.get<ProjectResponse>(`/projects/${id}`)
    ...
    async getProjectByName(orgName: string, projectName: string): Promise<Project | null> {
        const cacheKey = `${orgName}:${projectName}`;
        return this.requestCache.getOrSet<Project | null>(
            'ProjectByNameGET',
            cacheKey,
            async () => {
                for await (const page of this.getProjects(orgName, { name: { _eq: projectName } })) {
