...
export function mapRunResourceToDomain(run: any): Run {
  return {
    id: run.id,
...
export const resolvers = {
  Query: {
    ...
    run: async (_: unknown, { id }: { id: string }, { dataSources }: Context): Promise<Run | null> => {
      ...
      return runResource ? mapRunResourceToDomain(runResource) : null;
    }
  },
  Run: {
    workspace: async (run: Run, args: unknown, context: Context, info: any): Promise<Workspace | null> => {
      ...
      const wsResource = await context.dataSources.workspacesAPI.getWorkspace(workspaceId);
