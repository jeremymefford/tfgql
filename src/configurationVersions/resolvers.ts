...
export const resolvers = {
  Query: {
    configurationVersion: async (
      ...
      const cv =
        await ctx.dataSources.configurationVersionsAPI.getConfigurationVersion(
          id,
        );
      return cv;
    },
    ...
    workspacesWithConfigurationVersionsLargerThan: async (
      _: unknown,
      {
        ...
        excludeOrgs,
        bytes,
      }: { includeOrgs?: string[]; excludeOrgs?: string[]; bytes: number },
      ctx: Context,
    ): Promise<Workspace[]> => {
        ...
        for await (const workspacePage of ctx.dataSources.workspacesAPI.listWorkspaces(
          ...
          await parallelizeBounded(
            ...
            async (workspace: Workspace) => {
              const cvs =
                await ctx.dataSources.configurationVersionsAPI.listConfigurationVersions(
                  workspace.id,
                );
              await parallelizeBounded(
                cvs,
                async (cv: ConfigurationVersion) => {
                  const size = await resolvers.ConfigurationVersion.size(
                    cv,
                    {},
                    ctx,
                  );
  ...
  ConfigurationVersion: {
    size: async (
      ...
      return ctx.dataSources.configurationVersionsAPI.getConfigurationVersionSize(
        cv.downloadUrl,
      );
    },
    ingressAttributes: async (
      ...
      return ctx.dataSources.configurationVersionsAPI.getIngressAttributes(
        cv.id,
      );
    },
  },
